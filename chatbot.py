...
doc_chain = load_qa_chain(llm=llm, chain_type="stuff", prompt=PROMPT, verbose=False)
...
# Create Vector DB
vectordb = FAISS.from_documents(documents=doc, embedding=embedding_model)

# Save the Vector DB
vectordb.save_local(
    folder_path=os.path.join(vector_path, db_file_name), index_name="nlp"
)
...
def make_source_docs_serializable(source_docs):
    ...
    for doc in source_docs:
        if hasattr(
            doc, "metadata"
        ):  # If the doc has metadata, extract only simple types (like strings or numbers)
...
def ask(prompt_question):
    ...
    )  # Use chain.invoke to generate the answer to suppress warnings
    answer_text = answer["answer"].strip().rstrip("\n")  # Clean up the answer text
    answer_text = answer_text.replace("\n", " ")  # Remove newline
    answer_text = " ".join(answer_text.split())  # Remove double whitespaces
    serializable_sources = make_source_docs_serializable(
        answer.get("source_documents", [])
    )  # Convert the source_documents to only include serializable metadata
    return {"answer": answer_text, "source_documents": serializable_sources}
